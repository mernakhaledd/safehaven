...
export default function ReceiverSettings() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const remindersVal = await secureStore.getItem('pref_reminders_enabled');
        if (remindersVal !== null) setRemindersEnabled(remindersVal === 'true');
      } catch (e) {
        console.error('[Safe Haven] failed to load receiver preferences', e);
      }
    })();
  }, []);
  ...
  async function toggleReminders(val: boolean) {
    ...
    await secureStore.setItem('pref_reminders_enabled', String(val));
  }

  async function onSignOut() {
    try {
      console.log('[Safe Haven] Signing out receiver...');
        ...
        <Card style={dynamicBg(styles.card)}>
          ...
          <View style={styles.row}>
            <View style={{ flex: 1 }}>
              ...
              <Text style={[styles.rowSubtitle, dynamicText(12), highContrast && { color: '#444' }]}>Receive warnings on caregiver notifications or checks.</Text>
            </View>
            <Switch
              value={pushEnabled}
          ...
          <View style={styles.row}>
            <View style={{ flex: 1 }}>
              <Text style={[styles.rowTitle, dynamicText(15)]}>Scheduled Medicine Reminders</Text>
              <Text style={[styles.rowSubtitle, dynamicText(12), highContrast && { color: '#444' }]}>Enable custom reminders for daily medications.</Text>
            </View>
            <Switch
