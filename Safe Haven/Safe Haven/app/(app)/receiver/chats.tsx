...
export default function ReceiverChats() {
  ...
  const loadConversations = useCallback(async (silent = false) => {
    ...
    try {
      ...
      const { data: convs } = await supabase
        .from('conversations')
        .select('id, care_giver_profile_id')
        .eq('care_receiver_profile_id', activeProfile.id);

      const list: ConversationPreview[] = [];
      const tempDates: { [key: string]: string } = {};

      for (const partner of partners) {
        const conv = (convs ?? []).find((c) => c.care_giver_profile_id === partner.id);
        let conversationId = conv?.id;

        if (!conversationId) {
          const { data: rpcId, error: rpcErr } = await supabase.rpc('get_or_create_conversation', {
            p_giver_profile_id: partner.id,
      ...
      console.error('[Safe Haven] failed to load receiver conversations', err);
    } finally {
  ...
  useEffect(() => {
    const channel = supabase
      .channel('receiver_chats_list_realtime')
        ...
        <Screen style={{ paddingBottom: 0, backgroundColor: highContrast ? '#ffffff' : undefined }}>
          ...
          <View style={[styles.chatHeader, highContrast && { borderBottomWidth: 2, borderColor: '#000' }]}>
            ...
            <View style={styles.headerTitleWrap}>
              ...
              <Text style={[styles.headerSubtitle, dynamicText(11), { color: Colors.muted }]}>Caregiver</Text>
            </View>
            <View style={{ width: 60 }} />
          </View>
    ...
    <Screen style={{ backgroundColor: highContrast ? '#ffffff' : undefined }}>
      ...
      {loadingList && conversations.length === 0 ? (
        ...
        <View style={styles.center}>
          ...
          <Text style={[styles.emptyText, dynamicText(14)]}>Linked caregivers will show up here to chat.</Text>
        </View>
      ) : (
        <FlatList
          data={conversations}
